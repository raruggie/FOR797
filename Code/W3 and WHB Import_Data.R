...
###################################################################
### Combine W3 Training and Testing with WHB
###################################################################

WHB2 <- WHB
dfHydroGrab2 <- dfHydroGrab[dfHydroGrab$Sample.Name == 'WHB', ]

setnames(WHB2, old = c("Date.Time.EST"), new = c("DATETIME"))
WHB2$BkpSenseDate <- WHB2$DATETIME
dfHydroGrab2$BkpGrabDate <- dfHydroGrab2$DATETIME
WHB2 <- as.data.table(WHB2)[as.data.table(dfHydroGrab2), on = .(DATETIME), roll = TRUE]
WHB2$SenseTimeDiff <- abs(WHB2$BkpSenseDate - WHB2$BkpGrabDate)
WHB2 <- subset(WHB2, SenseTimeDiff  < 1800)
WHB2 <- WHB2 %>% drop_na()
WHB2 <- as.data.frame(WHB2)


GOF2 <- GOF
dfHydroGrab2 <- dfHydroGrab[dfHydroGrab$Sample.Name == 'GOF', ]

setnames(GOF2, old = c("Date.Time.EST"), new = c("DATETIME"))
GOF2$BkpSenseDate <- GOF2$DATETIME
dfHydroGrab2$BkpGrabDate <- dfHydroGrab2$DATETIME
GOF2 <- as.data.table(GOF2)[as.data.table(dfHydroGrab2), on = .(DATETIME), roll = TRUE]
GOF2$SenseTimeDiff <- abs(GOF2$BkpSenseDate - GOF2$BkpGrabDate)
GOF2 <- subset(GOF2, SenseTimeDiff  < 1800)
GOF2 <- GOF2 %>% drop_na()
GOF2 <- as.data.frame(GOF2)

colnames(GOF2)
colnames(W3TrainingData)

TempW3 <- as.data.frame(W3TrainingData)

W3HybridTraining <- TempW3[ , names(TempW3) %in% c("FDOM_corrected_QSU", "NO3_corrected_mgL", 
                                                                   "SpConductivity", "TempC", "DOYSin", "DOYCos", "Cl", 
                                                                   "NO3", "SO4", "Na", "K", "Mg", "Ca", "NH4", "DON", "PO4")]

GOF23 <- GOF2[ , names(GOF2) %in% c("FDOM_corrected_QSU", "NO3_corrected_mgL", 
                                                       "SpConductivity", "TempC", "DOYSin", "DOYCos", "Cl", 
                                                       "NO3", "SO4", "Na", "K", "Mg", "Ca", "NH4", "DON", "PO4")]

WHB23 <- WHB2[ , names(WHB2) %in% c("FDOM_corrected_QSU", "NO3_corrected_mgL", 
                                    "SpConductivity", "TempC", "DOYSin", "DOYCos", "Cl", 
                                    "NO3", "SO4", "Na", "K", "Mg", "Ca", "NH4", "DON", "PO4")]


tempHybrid <- rbind(W3HybridTraining, GOF23)

W3GOFWHBTraining <- rbind(tempHybrid, WHB23)
...
rm(list = c('drop', 'Temporarydf', 'TemporaryList', 'TemporaryDis', 
            ...
            'dfHydroGrab2', 'GOF2', 'GOF23', 'tempHybrid', 'TempW3', 'W3HybridTraining', 'WHB2', 'WHB23'))



## Save:

save.image(file = 'Processed_Data/W3GOFWHBImport_Data.Rdata')
