...
load(file = 'Processed_Data/W3GOFWHBData&Models.Rdata')
...
###
#data_test <- dfSensorGrab[ , c("NO3_corrected_mgL", "NO3", "PredNO3")]
data_test <- dfSensorGrab[ , c("SpConductivity", "Na")]

#data_test$Pred <- predict(lmPred, data_test)
lmPred2 = lm(Na ~ SpConductivity, data = data_test)
summary(lmPred2)
fig<-plot_ly(data = data_test, x = ~SpConductivity, y = ~Na, type = 'scatter')
fig <- fig %>% layout(title = varWatershed, xaxis = list(title = 'Grab Sample (mg/L)'), 
                      yaxis = list(title = 'Pred (mg/L)'))
fig
###
#x = ~NO3_corrected_mgL
