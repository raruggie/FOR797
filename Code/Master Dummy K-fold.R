...
load('Processed_Data/Master_Dummy_Import_Data.Rdata')
...
for (i in seq_along(1:length(YSolutes))) {
  ...
  for (j in seq_along(folds)) {
    ...
    #dfTraining <- dfMaster[dfMaster$Watershed != "DCF"]
    insample <- dfMaster[dfMaster$Watershed != "W3"]
    #insample <- do.call("rbind", list(dfTraining, insample))
    #insample <- Training_DCF
    #insample <- dfTraining
    
    TrainX_Dumb <- insample[ , c("FDOM_corrected_QSU", "NO3_corrected_mgL", 
                                 "SpConductivity", "TempC", "DOYSin", "DOYCos",
                                 "StreamOrder", "MedianSpecConduct", "DrainageArea", 
                                 "ForestedArea", "PercentWetland")]
...
#write.csv(SumStats, file = 'Models/ProperTrainAndTestSplits/Master8020.csv')
write.csv(SumStats, file = 'Models/ProperTrainAndTestSplits/DummyW38020.csv')
