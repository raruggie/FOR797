...
folds <- create_folds(Training_LMP$Ca, k = 5, seed = 2734, m_rep = 3)
#flds <- createFolds(Training_LMP, k = 5, list = TRUE)
#multiflds <- createMultiFolds(dfMaster, k = 4, times = 3)

#repflds <- replicate(3, createFolds(Training_LMP, k = 5), simplify=FALSE)
...
for (j in seq_along(folds)) {
  ...
  for (i in colnames(TrainY)) {
    ...
    SelectedX$SpConductivity
    as.numeric(TrainY$NO3)
    testY <- TrainY$NO3
    testY
    str(SelectedX)
    result <- replicate(5, rfcv(myiris, iris$Species), simplify=FALSE)
    result <- replicate(5, rfcv(SelectedX, TrainY$NO3), simplify=FALSE)
