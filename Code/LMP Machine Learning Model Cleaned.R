...
dfHydroGrabLMP <- dfHydroGrab[dfHydroGrab$Sample.Name == varWatershed, ]
...
dfHydroGrabLMP$BkpGrabDate <- dfHydroGrabLMP$DATETIME
dfSensorGrab <- as.data.table(dfWatershed)[as.data.table(dfHydroGrabLMP), on = .(DATETIME), roll = TRUE]
...
TrainX_Dumb <- dfSensorGrab[ , c("FDOM_corrected_QSU", "NO3_corrected_mgL", "SpConductivity", "TempC", "DOYSin", "DOYCos")]

TrainY <- dfSensorGrab[ , c("Cl", "NO3", "SO4", "Na", "K", 
                            "Mg", "Ca", "NH4", "DON", "PO4")]
...
save.image(file = 'Processed_Data/LMPData&Models.Rdata')
