...
ListOfHydroShare <- list("BDC", "BEF", "DCF", "GOF", "HBF", 
                      "LMP", "MCQ", "SBM", "TPB", "WHB")
ListOfHydroData <- list(BDC, BEF, DCF, GOF, HBF, 
                         LMP, MCQ, SBM, TPB, WHB)
...
for (i in 1:length(ListOfHydroShare)) {
  ...
  #print(summary(ListOfHydroData[[i]]))
  TempHydroGrab <- TempHydroGrab[TempHydroGrab$Sample.Name == ListOfHydroShare[[i]], ]
  setnames(ListOfHydroData[[i]], old = c("Date.Time.EST"), new = c("DATETIME"), skip_absent=TRUE)
  ...
  #dfSensorGrab <- dfSensorGrab[ , names(dfSensorGrab) %in% c("DATETIME", "FDOM_corrected_QSU", "NO3_corrected_mgL", "SpConductivity", "TempC", "DOYSin", "DOYCos", 
  #                                                           "DOC", "Cl", "NO3", "SO4", "Na", "K", "Mg", "Ca", "NH4", "DON", "PO4")]
  dfSensorGrab <- subset(dfSensorGrab, select = c("DATETIME", "FDOM_corrected_QSU", "NO3_corrected_mgL", "SpConductivity", "TempC", "DOYSin", "DOYCos", 
                                                  "DOC", "Cl", "NO3", "SO4", "Na", "K", "Mg", "Ca", "NH4", "DON", "PO4"))
...
W1TrainingData <- subset(W1TrainingData, select = c("DATETIME", "FDOM_corrected_QSU", "NO3_corrected_mgL", "SpConductivity", "TempC", "DOYSin", "DOYCos", 
                                                "DOC", "Cl", "NO3", "SO4", "Na", "K", "Mg", "Ca", "NH4", "DON", "PO4"))
W1TrainingData <- W1TrainingData %>% mutate(Watershed = "W1")

W3TrainingData <- subset(W3TrainingData, select = c("DATETIME", "FDOM_corrected_QSU", "NO3_corrected_mgL", "SpConductivity", "TempC", "DOYSin", "DOYCos", 
                                                    "DOC", "Cl", "NO3", "SO4", "Na", "K", "Mg", "Ca", "NH4", "DON", "PO4"))
W3TrainingData <- W3TrainingData %>% mutate(Watershed = "W3")

W9TrainingData <- subset(W9TrainingData, select = c("DATETIME", "FDOM_corrected_QSU", "NO3_corrected_mgL", "SpConductivity", "TempC", "DOYSin", "DOYCos", 
                                                    "DOC", "Cl", "NO3", "SO4", "Na", "K", "Mg", "Ca", "NH4", "DON", "PO4"))
W9TrainingData <- W9TrainingData %>% mutate(Watershed = "W9")


W1TrainingData <- W1TrainingData %>% mutate(StreamOrder = "1")
W1TrainingData <- W1TrainingData %>% mutate(MedianSpecConduct = "15.8")
W1TrainingData <- W1TrainingData %>% mutate(DrainageArea = "0.1")
W1TrainingData <- W1TrainingData %>% mutate(ForestedArea = "100")
W1TrainingData <- W1TrainingData %>% mutate(PercentWetland = "0")

W3TrainingData <- W3TrainingData %>% mutate(StreamOrder = "1")
W3TrainingData <- W3TrainingData %>% mutate(MedianSpecConduct = "15.8")
W3TrainingData <- W3TrainingData %>% mutate(DrainageArea = "0.4")
W3TrainingData <- W3TrainingData %>% mutate(ForestedArea = "100")
W3TrainingData <- W3TrainingData %>% mutate(PercentWetland = "0")

Training_BDC <- Training_BDC %>% mutate(StreamOrder = "0")
Training_BDC <- Training_BDC %>% mutate(MedianSpecConduct = "260")
Training_BDC <- Training_BDC %>% mutate(DrainageArea = "0.3")
Training_BDC <- Training_BDC %>% mutate(ForestedArea = "23.9")
Training_BDC <- Training_BDC %>% mutate(PercentWetland = "8.9")

Training_BEF <- Training_BEF %>% mutate(StreamOrder = "3")
Training_BEF <- Training_BEF %>% mutate(MedianSpecConduct = "18")
Training_BEF <- Training_BEF %>% mutate(DrainageArea = "13.7")
Training_BEF <- Training_BEF %>% mutate(ForestedArea = "98.4")
Training_BEF <- Training_BEF %>% mutate(PercentWetland = "0")

Training_DCF <- Training_DCF %>% mutate(StreamOrder = "1")
Training_DCF <- Training_DCF %>% mutate(MedianSpecConduct = "51")
Training_DCF <- Training_DCF %>% mutate(DrainageArea = "7")
Training_DCF <- Training_DCF %>% mutate(ForestedArea = "71.9")
Training_DCF <- Training_DCF %>% mutate(PercentWetland = "11.7")

Training_GOF <- Training_GOF %>% mutate(StreamOrder = "7")
Training_GOF <- Training_GOF %>% mutate(MedianSpecConduct = "95")
Training_GOF <- Training_GOF %>% mutate(DrainageArea = "7987.5")
Training_GOF <- Training_GOF %>% mutate(ForestedArea = "75.4")
Training_GOF <- Training_GOF %>% mutate(PercentWetland = "6.1")

Training_HBF <- Training_HBF %>% mutate(StreamOrder = "1")
Training_HBF <- Training_HBF %>% mutate(MedianSpecConduct = "13")
Training_HBF <- Training_HBF %>% mutate(DrainageArea = "0.4")
Training_HBF <- Training_HBF %>% mutate(ForestedArea = "99.3")
Training_HBF <- Training_HBF %>% mutate(PercentWetland = "0")

Training_LMP <- Training_LMP %>% mutate(StreamOrder = "6")
Training_LMP <- Training_LMP %>% mutate(MedianSpecConduct = "138")
Training_LMP <- Training_LMP %>% mutate(DrainageArea = "476.7")
Training_LMP <- Training_LMP %>% mutate(ForestedArea = "67.4")
Training_LMP <- Training_LMP %>% mutate(PercentWetland = "12.4")

Training_MCQ <- Training_MCQ %>% mutate(StreamOrder = "1")
Training_MCQ <- Training_MCQ %>% mutate(MedianSpecConduct = "659")
Training_MCQ <- Training_MCQ %>% mutate(DrainageArea = "0.4")
Training_MCQ <- Training_MCQ %>% mutate(ForestedArea = "9.1")
Training_MCQ <- Training_MCQ %>% mutate(PercentWetland = "2")

Training_SBM <- Training_SBM %>% mutate(StreamOrder = "1")
Training_SBM <- Training_SBM %>% mutate(MedianSpecConduct = "32")
Training_SBM <- Training_SBM %>% mutate(DrainageArea = "0.3")
Training_SBM <- Training_SBM %>% mutate(ForestedArea = "97.9")
Training_SBM <- Training_SBM %>% mutate(PercentWetland = "0")

Training_TPB <- Training_TPB %>% mutate(StreamOrder = "1")
Training_TPB <- Training_TPB %>% mutate(MedianSpecConduct = "20")
Training_TPB <- Training_TPB %>% mutate(DrainageArea = "4.1")
Training_TPB <- Training_TPB %>% mutate(ForestedArea = "88.6")
Training_TPB <- Training_TPB %>% mutate(PercentWetland = "6.1")

Training_WHB <- Training_WHB %>% mutate(StreamOrder = "1")
Training_WHB <- Training_WHB %>% mutate(MedianSpecConduct = "357")
Training_WHB <- Training_WHB %>% mutate(DrainageArea = "1")
Training_WHB <- Training_WHB %>% mutate(ForestedArea = "58.1")
Training_WHB <- Training_WHB %>% mutate(PercentWetland = "6.8")



WatershedLst <- list(W1TrainingData, W3TrainingData, Training_BDC,
...
StreamOrder <- list(1,1,0,3,1,7,1,6,1,1,1,1)
MedianSpecConduct <- list(15.8, 15.8, 260, 18, 51, 95, 13, 138, 659, 32, 20, 357)
DrainageArea <- list(0.1, 0.4, 0.3, 13.7, 7, 7987.5, 0.4, 476.7, 0.4, 0.3, 4.1, 1)
ForestedArea <- list(100, 100, 23.9,98.4, 71.9, 75.4, 99.3, 67.4, 9.1, 97.9, 88.6, 58.1)
PercentWetland <- list(0, 0, 8.9, 0, 11.7, 6.1, 0, 12.4, 2, 0, 6.1, 6.8)
...
for (i in seq_along(1:length(WatershedLst))) {
  
  #print(WatershedLst[i])
  print(StreamOrder[[i]])
  StrOrd <- StreamOrder[[i]]
  print(StrOrd)
  #print(MedianSpecConduct[[i]])
  #print(DrainageArea[[i]])
  #print(ForestedArea[[i]])
  #print(PercentWetland[[i]])
  
  WatershedLst[i] <- WatershedLst[i] %>% mutate(StreamOrder = StrOrd)
  #WatershedLst[i] <- WatershedLst[i] %>% mutate(MedianSpecConduct = MedianSpecConduct[[i]])
  #WatershedLst[i] <- WatershedLst[i] %>% mutate(DrainageArea = DrainageArea[[i]])
  #WatershedLst[i] <- WatershedLst[i] %>% mutate(ForestedArea = ForestedArea[[i]])
  #WatershedLst[i] <- WatershedLst[i] %>% mutate(PercentWetland = PercentWetland[[i]])
...
dfMaster <- do.call("rbind", list(W1TrainingData, W3TrainingData, Training_BDC,
                                  Training_BEF, Training_DCF, Training_GOF, Training_HBF, 
                                  Training_LMP, Training_MCQ, Training_SBM, Training_TPB,
                                  Training_WHB))
...
save.image(file = 'Processed_Data/Master_Dummy_Import_Data.Rdata')
